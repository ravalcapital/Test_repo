...
class UpdateLoanInstallment(models.Model):
    ...
    # @api.onchange('is_change_duration')
    # def _onchange_is_change_duration(self):
    #     print(f'in _onchange_is_change_duration')
    #     for rec in self:
    #         rec.is_change_int_rate = True
    #         # rec.interest_rate = loan_ids.int_rate
    ...
    #                                 # first_beginning_balance = installment_paid2_ordered[0].ending_balance
    #                             else:
    #                                 beginning_balance = ending_balance
    #                                 # first_beginning_balance = ending_balance
    #                         else:
    #                             print(
    ...
    #                             beginning_balance = ending_balance
    #                             # first_beginning_balance = beginning_balance
    #                             # interest_amt = round(((beginning_balance * (loan.int_rate / 100)) / 12), 2)
    #                             # print(
    ...
    #                     else:
    #                         beginning_balance = loan.principal_amount
    #                         # first_beginning_balance = loan.principal_amount
    #                     # principal_amt = round((loan.principal_amount / loan.duration), 2)
    #                     # =========== first_beginning_balance ====================
    #                     if installment_paid2_ordered:
    #                         first_beginning_balance = installment_paid2_ordered[0].ending_balance
    #                         principal_amt = round((first_beginning_balance / (loan.duration-len(installment_paid2_ordered))), 2)
    #                         print(f'in caseeee installment_paid2_ordered - first_beginning_balance :{first_beginning_balance} - (loan.duration-len(installment_paid2_ordered))) :{(loan.duration-len(installment_paid2_ordered))} - principal_amt :{principal_amt}')
    #                     else:
    #                         principal_amt = round((loan.principal_amount / loan.duration), 2)
    #                     # ================ end first_beginning_balance =============
    #                     interest_amt = round(((beginning_balance * (loan.int_rate / 100)) / 12), 2)
    #                     print(f'in fixed_principal loan.int_rate > 0.0 - beginning_balance :{beginning_balance} -  - loan.int_rate:{loan.int_rate} - interest_amt:{interest_amt} ')
    ...
    def _get_installment_values(self, loan, duration, paid_installment_line_ids):
        print(
            f'in _get_installment_values - self:{self} - loan :{loan} - loan.loan_type:{loan.loan_type} -  loan.loan_type.code:{loan.loan_type.code}-   duration :{duration} - paid_installment_line_ids :{paid_installment_line_ids}')
        if loan.loan_type.code =='AH FX':
            print(f'in _get_installment_values - loan.loan_type.code == AH FX ')
        principal_amt = loan.principal_amount - sum(paid_installment_line_ids.mapped("principal_amt"))
        interest_amt = total = 0.0
        reducing_val = {}
        loan_principal_amnt = principal_amt
        if loan.int_payable:
            ...
            if loan.interest_mode == 'flat':
                principal_amt = loan_principal_amnt / (duration - len(paid_installment_line_ids) or 1.0)
                total_interest_amount = loan.flat_rate_method(loan_principal_amnt, interest_rate,
                                                              duration - len(paid_installment_line_ids))
                interest_amt = total_interest_amount / (duration - len(paid_installment_line_ids) or 1.0)
                total = principal_amt + interest_amt
            if loan.interest_mode == 'reducing':
                if interest_rate > 0.0:
                    reducing_val = loan.reducing_balance_method(loan_principal_amnt, interest_rate,
                                                                duration - len(paid_installment_line_ids))
            # =========================== new code for Fixed Principal ================
                ...
                for install in loan.installment_lines:
                    ...
                    if loan.interest_mode == 'fixed_principal':
                        ...
                        if installment_paid2_ordered:
                            first_beginning_balance = installment_paid2_ordered[0].ending_balance
                            principal_amt = round(
                                (first_beginning_balance / (loan.duration - len(installment_paid2_ordered))), 2)
                            print(
                                f'in caseeee installment_paid2_ordered - first_beginning_balance :{first_beginning_balance} - (loan.duration-len(installment_paid2_ordered))) :{(loan.duration - len(installment_paid2_ordered))} - principal_amt :{principal_amt}')
                        else:
                            principal_amt = round((loan.principal_amount / loan.duration), 2)
                        # ================ end first_beginning_balance =============
                        interest_amt = round(((beginning_balance * (loan.int_rate / 100)) / 12), 2)
                        print(
                            f'in fixed_principal loan.int_rate > 0.0 - beginning_balance :{beginning_balance} -  - loan.int_rate:{loan.int_rate} - interest_amt:{interest_amt} ')
                        print(
                            f'case installment_paid2_ordered :{installment_paid2_ordered} - len(installment_paid2_ordered)- : {len(installment_paid2_ordered)} - loan.duration:{loan.duration}')
                        print(
                            f'in fixed_principal loan.int_rate > 0.0 - loan.principal_amount :{loan.principal_amount} -  - beginning_balance:{beginning_balance} - - loan.int_rate :{loan.int_rate} - loan.duration:{loan.duration} ')
                        total = round((principal_amt + interest_amt), 2)
                        ending_balance = round((beginning_balance - principal_amt), 2)
                        ...
                        install.write({'principal_amt': principal_amt,
                                       ...
                                       'total': total})
                    if loan.loan_type.code =='AH FX':
                        print(f'in case fixed_principal and loan.loan_type.code = AH FX ')
                        loan.recalculate_alhaly_loan()
            # =========================== End new code for Fixed Principal ============
            print(
                f'in _get_installment_values jojo -  vals before return - principal_amt:{principal_amt} - interest_amt:{interest_amt} - total:{total} - reducing_val:{reducing_val}')
        return principal_amt, interest_amt, total, reducing_val
...
#         for install_mnt in installment_line_ids:
#             if duration < install_no:
#                 print(f'case duration < install_no = duration :{duration} - install_no :{install_no}')
#                 install_mnt.write({
#                     'active': False,
#                 })
#                 continue
...
#         return term_change_history_vals
    ...
    def action_update_installment2(self):
        print(f'in action_update_installment_2')
    ...
    #         })
    #         self.env['term.change.history'].create(term_change_history_vals)
    #     # self.action_update_installment_2()
    #     if recalc_interest_rate==False and self.duration and self.is_change_duration:
    #         change_loan_term_installment = self.env['change.loan.term.installment']
    #         vals = {
    #             'is_change_int_rate':True,
    #             'interest_rate':loan_ids.int_rate,
    #             'is_confirm_change_term':True
    #         }
    #         new_wizard = change_loan_term_installment.create(vals)
    #         new_wizard.action_update_installment2()
    #         recalc_interest_rate = True
    #     return True
