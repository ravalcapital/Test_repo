...
class PartnerLoanType(models.Model):
    ...
    interest_mode = fields.Selection(selection_add=[('mortgage_loan', "Mortgage Loan")], ondelete={'mortgage_loan': 'set null'})


class PartnerLoanInstallmentDetails(models.Model):
    _inherit = 'partner.loan.installment.details'

    beginning_balance = fields.Float(string='Beginning Amount')
    ending_balance = fields.Float(string='Ending Balance')
    #
    # @api.depends('loan_id', 'loan_id.principal_amount')
    # def get_beginning_balance(self):
    #     for rec in self:
    #         other_lines  = self.search([('loan_id', '=', self.loan_id.id)])
    #         if not other_lines and rec.loan_id.loan_type_id.interest_mode =='mortgage_loan':
    #                rec.beginning_balance = rec.loan_id.principal_amount
    #         else:
    #             other_lines_list = []
    #             for rec_loan in other_lines:
    #                 other_lines_list.append(rec_loan)
    #             last_line = max(other_lines_list)
    #             rec.beginning_balance = last_line.total
    ...
    def create_installments(self, loan):
        ...
        for install_no in range(0, loan.duration):
            ...
            if loan.interest_mode == 'mortgage_loan':
                ...
                if loan.int_rate > 0.0:
                    total = (loan.principal_amount * ((loan.int_rate/100) / loan.duration)) / (1 - ((1 + ((loan.int_rate/100) / loan.duration)) ** - loan.duration))
