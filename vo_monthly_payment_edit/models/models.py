...
class VehicleCarDetails(models.Model):
    ...
    def generate_pre_invoice_lines_action(self):
        ...
        for rec in self:
            ...
            vo_end_date = rec.vehicle_order_id.vo_end_date
            ...
            for pay_no in range(0, int(rec.no_of_invoices)):
                ...
                if vo_end_date:
                    if vo_end_date >= date_to and vo_end_date > date_from:
                        monthly_payment_obj.create(monthly_payment_line)
                    elif vo_end_date < date_to and vo_end_date > date_from :
                        date_to = vo_end_date
                        monthly_payment_line.update({'date_to':date_to.strftime('%Y-%m-%d')})
                        monthly_payment_obj.create(monthly_payment_line)
                else:
                    monthly_payment_obj.create(monthly_payment_line)
            ...
            if date_to_diff:
                ...
                ddd_date_to = date_to.strftime('%Y-%m-%d')
                monthly_payment_line = {
...
class VehicleCarDetailsMillage(models.Model):
    ...
    def create_invoice_action(self):
        print(f'in create_invoice_action of vo_monthly_payment_edit - millage.vehicle.payment ')
        for rec in self:
                    ...
                    if vo.extra_km > 0:
                        if vo.pool_millage != True:
                            ...
                            if vo.invoice_policy == 'grouped':
                                ...
                                rec.invoice_id = out_invoice_obj.id
                                lines_groued.invoice_id= out_invoice_obj.id
                                ...
                                if not there_is_km:
                                    ...
                                    if not out_invoice_obj_indvidual2_flag:
                                        ...
                                        out_invoice_obj_indvidual2_flag = out_invoice_obj_indvidual2
                                        vo.extra_km_invoice_ids = [(4, out_invoice_obj_indvidual2.id)]
                                        rec.invoice_id = out_invoice_obj_indvidual2.id
                    # rec.invoice_id = out_invoice_obj.id
