...
class PartnerLoanDetails(models.Model):
    ...
    def compute_installments(self):
        print(f'in compute_installments 222 ')
        for loan in self:
    ...
    def recalculate_alhaly_loan(self):
        print(f'ib recalculate_alhaly_loan - self:{self}')
        for rec in self:
            if rec.loan_type.code =='AH FX':
                print(f'in ib recalculate_alhaly_loan type is alhaly loannnnn - code :{rec.loan_type.code}' )
