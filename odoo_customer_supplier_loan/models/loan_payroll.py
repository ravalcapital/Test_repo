...
#                 debit_account_id = debit_account_id.id
    ...
    #                     'period_id': period_id,
                ...
                installment.write({'state':'booked'})

                print(f'in book_interest - installment state :{installment.state}')
...
#             debit_account_id = debit_account_id.id
            if not installment.loan_id.partner_loan_account:
                raise ValidationError( _('Please configure Partner account.'))
            deb_interest_line = (0, 0, {
                    'name': _('Interest of loan %s') % (installment.loan_id.name),
                    'date': installment.date_from,
                    'partner_id': partner_id,
...
#                     'period_id': period_id,
                ...
                if not installment.loan_id.journal_id1.default_account_id:
                    raise ValidationError( _('Please configure Debit/Credit accounts on the Journal %s ') % (installment.loan_id.journal_id1.name))
                if not installment.loan_id.partner_loan_account:
                    raise ValidationError( _('Please give partner account.'))
                debit_line = (0, 0, {
...
#                     'period_id': period_id,
                ...
                if not installment.loan_id.journal_id.default_account_id:
                    raise ValidationError( _('Please configure Debit/Credit accounts on the Journal %s ') % (self.journal_id.name))
                debit_line = (0, 0, {
                        'name': _('EMI of loan %s') % (installment.loan_id.name),
                        'date': installment.date_from,
                        'partner_id': partner_id,
    ...
    def _cal_amount_all(self):
        for rec in self:
            ...
            if rec.int_payable:
                ...
                #         print(f'in _cal_amount_all - values :{values}')
                #         #for key, value in values.iteritems():
                #         for key, value in values.items():
                #             rec.total_interest_amount += value['interest_comp']
                #
                total_principal_amt = 0.0
                for line in rec.installment_lines:
                    total_principal_amt += line.principal_amt
                rec.total_interest_amount = total_principal_amt
...
#                    else:
    ...
    def reducing_balance_method(self, p, r, n):
        ...
        print(f'in reducing_balance_method - res :{res}')
        return res
...
#                            break
    ...
    bank_journal_id_for_int  = fields.Many2one(
        'account.journal',
        'Bank/Cash Journal For Installment',
        copy=True,
        required=False
    )
...
#    )
    ...
    @api.onchange('journal_id2', 'journal_id')
    def onchange_journal_id_interest(self):
        for rec in self:
            if rec.journal_id2:
               rec.interest_account_id = rec.journal_id2.default_account_id.id
            if rec.journal_id:
               rec.disbursed_account_id = rec.journal_id.default_account_id.id
...
#         return super(EmployeeLoanDetails, self).copy(default)
    ...
    def onchange_loan_type(self, loan_type, partner):
        if loan_type:
            if not partner:
                raise ValidationError(_('Please specify partner.'))
            ...
            if partner not in allowed_partners:
                raise ValidationError( _('%s  does not Qualify for %s ') % (partner_obj.name, loan_type.name))
        return {}
...
#         department_id = partner_obj.department_id.id or False  #todo check
        ...
        if not address_id:
            raise ValidationError( _('There is no home/work address defined for partner : %s ') % (_(partner_obj.name)))
        partner_id = address_id and address_id.id or False
        if not partner_id:
            raise ValidationError( _('There is no partner defined for partner : %s ') % (_(partner_obj.name)))
        gross_amount = 0.0
#         if line_ids:
...
#                msg += 'Interest Rate\n '
            ...
            if msg:
                raise ValidationError( _('Please Enter values greater then zero:\n %s ') % (msg))
            status = self.check_employee_loan_qualification(loan)
            if not isinstance(status, bool):
                raise ValidationError( _('Loan Policies not satisfied :\n %s ') % (_(status)))
...
#                            % (policy.name, contract_date, policy.policy_value, actual_date.strftime('%Y-%m-%d'))
    ...
    def _get_installment_dates(self, year, month, day):
...
#                if loan.int_payable:
    ...
    def _interest_journal(self, loan, move_inrest, vals, partner_id, move_pool):
        ...
        if not loan.journal_id2:
                raise ValidationError(_('Please configure Interest Journal.'))
        if not loan.interest_account_id:
            raise ValidationError(_('Please configure Debit/Credit accounts on the Interest Journal %s ') % (loan.journal_id2.name))
        credit_account_id = loan.interest_account_id.id
        debit_account_id = loan.interest_receivable_account_id.id or False
        if not debit_account_id:
            raise ValidationError(_('Please configure debit account of Interest'))
...
#                 'period_id': period_id.id,
            ...
            if not credit_account_id:
                raise ValidationError(_('Please configure Debit/Credit accounts on the Journal %s ') % (loan.journal_id.name))
